    ...
    try {
        ...
        for (let i = 0; i < count; i++) {
            try {
                ...
                const response = await fetch(`${process.env.FLUX}`, options);
...
app.post('/api/v1/user/register', async (req, res) => {
    ...
    if (!username || !email || !password) return res.json({ error: 'All user details are required..!' });
...
app.get('/api/v1/get-conversation/:conversationId/:_id', (req, res) => {
    ...
    Conversation.find({ conversationId: conversationId, userId: _id })
        .then(conversations => {
...
const deleteNullImageConversations = () => {

    Conversation.deleteMany({ images: [] })
        .catch(error => {
            console.error('Error deleting conversations with null or empty images:', error);
        });
